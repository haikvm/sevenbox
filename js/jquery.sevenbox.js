...
(function ($, document, window) {
	...
	publicMethod = $.fn[sevenbox] = $[sevenbox] = function (options, callback) {
		console.log($box);
		var $ob = this;
		options = options || {};
		if (callback) {
			options.onAfterOpen = callback;
		}
		if ($.isFunction($ob)) { // assume a call to $.colorbox
			$ob = $('<a/>');
			options.open = true;
		}
		settings = new Settings(options);
		console.log($ob);
		$ob.addClass(sevenbox);
		...
		if ($.isFunction($.fn.on)) {
			$(document).on('click', '.'+sevenbox, clickHandler); // For jQuery 1.7+
		} else {
			$('.'+sevenbox).live('click', clickHandler); // For jQuery <1.6.x
		}
	...
	init = function(el) {
		if (!$box) {
			settings.get('onBeforeOpen');
			$window = $(window);
			$bodyContent = $('body').html().replace(/(<script)(.*?)(<\/script>)/gi, '');;
	...
	function destroy() {
		settings.get('onBeforeClose');
		$overlay.remove();
		$('.'+sevenbox).unbind('click');
		$box.fadeOut(function() {
			$(this).remove();
			$box = false;
			$overlay = false;
			settings.get('onAfterClose');
		});
	}
